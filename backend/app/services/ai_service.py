...
class AIService:
    ...
    async def _save_interaction(
        ...
        # Refresh to keep ORM objects accessible after commit
    ...
    def _build_context(self, session: Session, latest: Interaction) -> str:
        """Build context string by appending the latest interaction to existing context."""
        # Avoid lazy-loading the relationship; just append the latest to the stored context
        existing = session.context_data or ""
        # intent is stored as a plain string value (e.g. 'chat') not an enum
        intent_str = latest.intent if isinstance(latest.intent, str) else latest.intent.value
        latest_summary = f"[{intent_str}] {latest.response or ''}"
        if existing:
            # Keep last 2 context lines, then append new one (limit history to 3 entries)
    ...
    async def ask_question(
        ...
        async def _do(db_: AsyncSession):
            result = await db_.execute(select(Session).where(Session.id == session_id))
            session = result.scalar_one_or_none()
            if not session:
            ...
            interaction = await self._save_interaction(
                db_, session, InputType.QUESTION, adjusted_response.intent,
                question, adjusted_response, final_decision,
                [input_meta], adjusted_response.processing_time_ms,
                request_id or str(uuid.uuid4())
            )
