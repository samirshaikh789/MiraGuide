...
class AccessibilityDecisionEngine:
    ...
    async def process_request(
        ...
        """
        # Get session context
        # Note: We intentionally do NOT access session.interactions here because
        # that would trigger an async lazy-load which breaks greenlet context.
        # The context_data string already encodes the recent interaction history.
        ...
        intent, intent_confidence = await self.detect_intent(
            question, has_image, has_document, session, has_form
        )
        ...
        context = DecisionContext(
            ...
            previous_intent=None,  # Not loaded to avoid lazy-load on async session
