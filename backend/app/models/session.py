...
class Interaction(Base):
    ...
    # Input — stored as plain strings (enum values) for cross-DB compatibility
    input_type: Mapped[str] = mapped_column(String(16), nullable=False)
    intent: Mapped[str] = mapped_column(String(20), default=IntentType.UNKNOWN.value)
    ...
    def __repr__(self) -> str:
        return f"<Interaction(id={self.id}, intent={self.intent})>"
...
class InputMetadata(Base):
    ...
    processing_status: Mapped[str] = mapped_column(
        String(16), default=ProcessingStatus.PENDING.value
    )
