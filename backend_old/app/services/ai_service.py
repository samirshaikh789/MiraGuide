...
class AIService:
    ...
    async def _get_or_create_session(
        ...
        # Create new session
        session = Session(user_id=user_id or uuid.uuid4())
        db.add(session)
        await db.flush()
        return session
    ...
    async def analyze_image(
        ...
        async for db in get_db():
            ...
            if not provider_result.success:
                ...
                error_response = StructuredAIResponse(
                    intent=intent_type,
                ...
                decision.clarification_question = error_response.clarification_question
            else:
                error_response = provider_result.data
    ...
    async def analyze_form(
        ...
        async for db in get_db():
            ...
            # Use vision provider for form analysis
            provider_result = await self._run_provider_with_fallback(
                "vision", "analyze_image",
                image_data, mime_type,
                prompt="Analyze this form. Identify all fields, labels, types, required status, and instructions.",
    ...
    async def simplify_text(
        ...
        async for db in get_db():
            ...
            interaction = await self._save_interaction(
                db, session, InputType.TEXT, adjusted_response.intent,
                None, adjusted_response, final_decision,
                [input_meta], adjusted_response.processing_time_ms,
                request_id or str(uuid.uuid4())
            )
